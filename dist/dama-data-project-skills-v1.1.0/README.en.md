...
To use the suite in **another data project**, copy or link the complete skill directories you need from this repository's skills/ into that project's .agents/skills/. Preserve each directory's SKILL.md, references/, agents/, and templates/; copying only SKILL.md loses supporting guidance and deliverables. Install the router alongside the relevant domain skills for cross-domain projects; a focused task may need only one domain skill. If a newly added skill does not appear, restart Codex. See the [official Codex skill documentation](https://learn.chatgpt.com/docs/build-skills) for discovery locations and explicit or implicit invocation.

## Deliverable templates

Every skill includes six ready-to-copy files in its own `templates/` directory: a Chinese and English Markdown delivery pack (`delivery-pack.zh.md` / `.en.md`), Word report (`report.zh.docx` / `.en.docx`), and Excel working register (`register.zh.xlsx` / `.en.xlsx`). Use Markdown for the complete handoff, Word for narrative analysis and review, and Excel for matrices, evidence registers, and scorecards. The same stable field IDs connect both languages and all formats. Start with the router's [English delivery pack](skills/managing-data-projects/templates/delivery-pack.en.md) or [Chinese delivery pack](skills/managing-data-projects/templates/delivery-pack.zh.md). Blank fields are intentional: confirm project facts, owners, thresholds, approvals, and evidence; do not treat the template as an approved decision. Copy the full skill directory to keep its templates portable. The `skills/` files are editable source; `dist/` contains versioned distribution copies.
...
- [DMBOK Chapter 1–17 source map](skills/managing-data-projects/references/source-map.md); [cross-domain routing table](skills/managing-data-projects/references/routing-table.md); [completion audit and behavior evaluations](https://github.com/yonglun/dama-skill/blob/main/evals/skill-suite/completion-audit.md).
